...
itself, so nothing in CI requires `lot-0-review.md` … `lot-6-review.md` here. The
hardened guard does require them at `gh pr create` time from a lot-range branch;
PR #5 was opened before the check existed and is not retroactively blocked.
...
| 7 | Medium | `plugins/claude-harness/hooks/git-guard.py:34` | `LOT_BRANCH` captured only the first lot of a range and never checked `lot-N-review.md`, so the guard passed PRs that `lot-deliverables.yml` then rejects. A guard that is laxer than the CI it mirrors is noise | fixed in `5414fd4` |
